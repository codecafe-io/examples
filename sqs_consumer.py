...
class SQSConsumer (threading.Thread):
    ...
    def getQueue(self, sqsQueueName=QUEUE_NAME):
        ...
        try:
            ...
            queue = self.sqs.get_queue_by_name(QueueName=sqsQueueName)        #@Del
    ...
    def consumeMessages(self, sqsQueueName=QUEUE_NAME):
        ...
        try:
            ...
            if queue:
                ...
                mesgs =  queue.receive_messages(                                                        #@Del
                                        AttributeNames=['All'], MaxNumberOfMessages=10, WaitTimeSeconds=20)    #@Del
                if not len(mesgs):
                    print("There are no messages in Queue to display")
                    return numMsgs
                for mesg in mesgs:        
                    #STUDENT TODO: Retrieve the Attributes of a message.
                    attributes = mesg.attributes            #@Del
